...
document.addEventListener("DOMContentLoaded", function () {
  fetch(URL)
    .then((response) => response.json())// 1.data in json format
var URL = "https://billboardbackend.onrender.com/";
  ...
  fetch(URL)
    ...
    .then((data) => {
      ...
      tableData.forEach((object) => {
        ...
        var { dateString, Name, Trip, MarkerType, Distance, TotalFare } = object;
        ...
        var oldDate= dateObject.toLocaleDateString();
        const [month, date, year] = oldDate.split('/');
...
function removeMarkersOfType(markerType, isChecked) {
  if (isChecked) {
    // console.log(markerType.toLowerCase())
    setMapOnMarkerType(markerType, map);
    document.getElementById(`marker-type-${markerType.toLowerCase()}-label`).style.color = ""
  } else {
    // console.log(isChecked)
    setMapOnMarkerType(markerType, null);
    document.getElementById(`marker-type-${markerType.toLowerCase()}-label`).style.color = "black"
  }
}
